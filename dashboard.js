...
function showBanner(online) {
  ...
  if (online) {
    ...
    banner.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Server offline — showing demo data. Run: <code style="background:rgba(0,0,0,.3);padding:2px 6px;border-radius:4px;">cd student-portal && node server.js</code>';
  }
}
...
const PAGES = [
  ...
  { label:'Student Portal',    icon:'fa-graduation-cap',  url:'student-portal/public/login.html' },
  { label:'Urban Kicks Store', icon:'fa-shoe-prints',     url:'urban kicks/index.html' },
  { label:'Developer Portfolio',icon:'fa-code',           url:'portfolio/index.html' },
];
...
async function loadDashboard() {
  try {
    ...
    renderNews(noticesRes.notices.length ? newsRes.news : []);   // news from news endpoint
